...
cat > ~/Downloads/farmacia-tools/.github/scripts/fetch_zoho_data.py << 'PYEOF'
import requests, json, os, time
from datetime import datetime
...
def main():
    ...
    sql = "SELECT * FROM AROEIRA_PRODUCT_ANALYSIS"
    ...
    produtos.sort(key=lambda x: float(x.get("TOTAL_QT", 0) or 0), reverse=True)
